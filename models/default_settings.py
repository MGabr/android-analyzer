...
untrusted_ca = Certificate(
    'Signed on-the-fly by an untrusted CA',
    '''
    This is a not expired, on-the-fly generated certificate with the right hostname.
    It is signed by an untrusted certificate authority.
    ''',
    custom_ca="mitmproxy/ca.pem")

default_certificates = [mitmproxy_ca_signed, other_hostname_self_signed, untrusted_ca]
...
no_pinning_hv = ScenarioSettings(
    ...
    [],
    '''
    The app has a possibly vulnerable HostnameVerifier implementation and certificate pinning is not implemented.
    ''')
...
mitmproxy_ca_signed_wv = ScenarioSettings(
    ...
    [],
    '''
    The app has a vulnerable WebViewClient implementation and a man-in-the-middle attack with a certificate generated by a untrusted certificate authority (but otherwise valid) was possible.
    ''')
...
mitmproxy_self_signed_wv = ScenarioSettings(
    ...
    [],
    '''
    The app has a vulnerable WebViewClient implementation and a man-in-the-middle attack with a self signed certificate with another hostname (*.mitmproxy.org) was possible.
    ''')


default_scenarios = [mitmproxy_ca_signed_tm, other_hostname_self_signed_hv, no_pinning_hv, mitmproxy_ca_signed_wv, mitmproxy_self_signed_wv]
